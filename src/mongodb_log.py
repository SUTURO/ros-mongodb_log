...
from sensor_msgs.msg import PointCloud, CompressedImage, PointCloud2
...
class MongoWriter(object):
    ...
    def create_worker(self, idnum, topic, collname):
        ...
        elif not self.no_specific and msg_class == PointCloud2:
            print("DETECTED point cloud topic %s, using fast C++ logger" % topic)
            node_path = find_node(PACKAGE_NAME, "mongodb_log_pcl2")
            if not node_path:
                print("FAILED to detect mongodb_log_pcl2, falling back to generic logger (did not build package?)")
